...
class tabletojson extends AbstractToJson {
    ...
    static convert(html, options) {
        ...
        } else if (options.cheerioDecodeEntities === false) {
            options.decodeEntities = false;
        } else if (options.euc_kr === true) {
            options.euc_kr = true;
        }
        ...
        const $ = cheerio.load(html, { decodeEntities: options.decodeEntities });
    ...
    static async convertUrl(url, arg1, arg2) {
        ...
        if (typeof arg2 === 'function') {
            ...
            if (requestOptions.euc_kr) {
                let buf = new Buffer(html, 'binary')
                let data = iconv.convert(buf).toString()
                return callback.call(this, tabletojson.convert(data, options));
            }
